...
SESSION_COOKIE = "zenless_session"
...
class LocalWebBridge:
    ...
    async def _request_middleware(self, request: web.Request, handler: Handler) -> web.StreamResponse:
        ...
        try:
            ...
            if request.path.startswith("/api/") and request.path != "/api/session":
                header_token = request.headers.get("X-Zenless-Token", "")
                cookie_token = request.cookies.get(SESSION_COOKIE, "")
                if not (
                    secrets.compare_digest(header_token, self.token)
                    or secrets.compare_digest(cookie_token, self.token)
                ):
    ...
    async def _session(self, _request: web.Request) -> web.Response:
        response = self._json({"token": self.token})
        response.set_cookie(SESSION_COOKIE, self.token, httponly=True, samesite="Strict", path="/")
        return response
