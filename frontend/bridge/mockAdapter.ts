...
export function createMockAdapter() {
  ...
  const adapter = {
    ...
    // Visual
    getVisual() {
      return { views: state.views, concept: { version: 3, status: 'READY', prompt: state.conceptPrompt } };
    },
    approveVisual() { state.views = state.views.map((view) => ({ ...view, state: 'APPROVED' })); return { ok: true }; },
    editConcept(prompt?: string) { if (prompt) state.conceptPrompt = prompt; return { ok: true }; },
    ...
    lockStudioReference(id: string) { updateStudioNode(state.studioTree, id, (node) => { node.locked = true; }); return { ok: true }; },
    unlockStudioReference(id: string) { updateStudioNode(state.studioTree, id, (node) => { node.locked = false; }); return { ok: true }; },
    useStudioAsContext(id: string) { updateStudioNode(state.studioTree, id, (node) => { node.usedAsContext = true; }); return { ok: true }; },
    ...
    getModels() { return state.modelCatalog; },
...
function updateStudioNode(nodes: MockState['studioTree'], id: string, update: (node: MockState['studioTree'][number]) => void): boolean {
