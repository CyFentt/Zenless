...
import type { ContextItem, Review } from '@/types';
...
function ContextTab() {
  ...
  useEffect(() => {
    if (!currentJobId) { setContextItems([]); return; }
    getApi().getContext(currentJobId).then(setContextItems).catch((error) => frontendDiagnostics.capture(error, 'build', 'Failed to load context'));
  ...
  const handleAction = async (item: ContextItem, action: 'include' | 'exclude' | 'lock' | 'unlock' | 'inspect') => {
    ...
    if (action === 'inspect') {
      try {
        const inspected = await api.inspectContext(item.id);
        frontendDiagnostics.report('info', 'context', `${inspected.name} — ${inspected.path}`);
      } catch (error) { frontendDiagnostics.capture(error, 'context', 'Failed to inspect context'); }
    }
  };

  if (!currentJobId) return <div className="flex items-center justify-center h-full text-xs text-ink-400 uppercase">NO JOB</div>;
...
function ChangesTab() {
  ...
  useEffect(() => {
    if (!currentJobId) { setChangedFiles([]); setReview(null); return; }
    Promise.all([getApi().getChanges(currentJobId), getApi().getReview(currentJobId)])
      .then(([files, nextReview]) => { setChangedFiles(files); setReview(nextReview); })
      .catch((error) => frontendDiagnostics.capture(error, 'build', 'Failed to load changes/review'));
  }, [currentJobId, setChangedFiles]);

  const selectedFile = changedFiles.find((f) => f.id === selectedFileId);

  const handleApprove = async () => { if (currentJobId) await getApi().approveChanges(currentJobId); };
  const handleReject = async () => { if (currentJobId) await getApi().rejectChanges(currentJobId); };
        ...
        <div className="px-3 py-2 border-b border-ink-600">
          ...
          {review && (
            <div className="mt-1 flex items-center gap-2">
              <span className={`text-2xs font-medium ${review.decision === 'APPROVE' ? 'text-zen-okBright' : review.decision === 'BLOCK' ? 'text-zen-errBright' : 'text-zen-warnBright'}`}>{review.decision}</span>
              <span className="text-2xs text-ink-400">{review.risk}</span>
              {(review.criticalIssues.length + review.warnings.length) > 0 && <span className="text-2xs text-ink-500">{review.criticalIssues.length + review.warnings.length} ISSUES</span>}
            </div>
          )}
        </div>
