...
export function TestPage() {
  ...
  const filteredLogs =
    logFilter === "ALL" ? testLogs : testLogs.filter((l) => l.level === logFilter);
      ...
      <div className="flex items-center justify-between px-3 h-9 border-b border-ink-600 shrink-0">
        <div className="flex items-center gap-3">
          {testState.status === "RUNNING" ? (
            <button
              onClick={handleStop}
              className="flex items-center gap-1.5 px-2.5 h-7 text-2xs uppercase tracking-wider text-zen-errBright border border-ink-600 hover:bg-ink-800 transition-colors"
            >
              <Square size={10} /> STOP
            </button>
          ) : (
          ...
          <span
            className={`text-2xs uppercase tracking-wider ${testState.status === "RUNNING" ? "text-zen-okBright" : testState.status === "FAILED" ? "text-zen-errBright" : "text-ink-400"}`}
          >
            {testState.status}
          </span>
          {testState.status === "RUNNING" && (
          ...
          {testState.fixAttempt > 0 && (
            <span className="text-2xs text-zen-warnBright font-mono">
              FIX {testState.fixAttempt}/{testState.maxFixAttempts}
            </span>
          )}
          {testCases.length > 0 && (
            <span className="text-2xs text-ink-300 font-mono">
              CASES {passedCases}/{testCases.length}
            </span>
          )}
        </div>
      ...
      >
        {selectedLog && (
          <div className="space-y-3">
            ...
            {selectedLog.testCaseId && (
              <DetailRow label="TEST CASE" value={selectedLog.testCaseId} />
            )}
            {selectedLog.suite && <DetailRow label="SUITE" value={selectedLog.suite} />}
            {selectedLog.expected !== undefined && (
              <DetailRow label="EXPECTED" value={formatDetail(selectedLog.expected)} />
            )}
            {selectedLog.actual !== undefined && (
              <DetailRow label="ACTUAL" value={formatDetail(selectedLog.actual)} />
            )}
            {selectedLog.stack && (
              <div>
                <span className="text-2xs uppercase tracking-wider text-ink-300">STACK</span>
                <pre className="mt-1 p-2 bg-ink-950 border border-ink-700 text-2xs text-ink-100 overflow-x-auto scrollbar-zen">
                  {selectedLog.stack}
                </pre>
              </div>
            )}
